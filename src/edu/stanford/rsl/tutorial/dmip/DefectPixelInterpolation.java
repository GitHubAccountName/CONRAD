...
public class DefectPixelInterpolation {
	...
	public Grid2D interpolateSpectral(Grid2D image, Grid2D mask, int maxIter, boolean zeroPadding)
	{
		...
		for(int i = 0; i < maxIter; i++)
		{
			...
			if(maxDeltaE_G_Ratio <= maxDeltaE_G_Thresh)// Ratio :energy error
			{
				System.out.println("maxDeltaE_G_Ratio = " + maxDeltaE_G_Ratio);
				break;
			}
			
			//In the i-th iteration select line pair s1,t1 maximum changes
			//which maximizes the energy reduction [Paragraph after Eq. (16) in the paper] for two point with same maximum value
			double maxDeltaE_G = Double.NEGATIVE_INFINITY;			
			//create arraylist to store lines (in case multiple maxima are found)
			//TODO
			ArrayList<Integer[]> sj1 = null; //save the point with same energy in the array
			...
			int idx = (int) Math.floor(Math.random() * sj1.size());// random smaaller than one 
			...
			//mirror the positions at halfDim (not used)
				...
				(s1 == halfDim[0] && t1 == halfDim[1]))
			{
				...
				//Eq. 15
				//FHat = N*(G(s,t)/W(0,0))
				//TODO compute FHatNext, use Complex class //compute F(s)
	 ...
	 *          = (F(s,t)W(k1-s,k2-t) + F*(s,t)W(k1+s,k2+t)) / (MN)two values changed we can check that
	...
	public static void main(String[] args) {
		...
		String filename = "/proj/i5dmip/he34toza/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/testimg.bmp";
		String filenameMask = "/proj/i5dmip/he34toza/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/mask.bmp";
		...
		Grid2D defectImage = new Grid2D(image);
		 defectImage.getGridOperator().multiplyBy(defectImage, mask);//multiplyBy doesn't return value
		...
		boolean zeroPadding = true;// zeros at the end for high resolution
		int maxIter = 4000;
		
		//semetry deconvolution
